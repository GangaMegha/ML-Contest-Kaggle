...
with open("../Results/ab_prob_results.txt", "w") as text_file:
	...
	for i in range(5):
		...
		ab = AdaBoostClassifier(DecisionTreeClassifier(max_depth=3), n_estimators=700, learning_rate=0.1)
		ab.fit(train_data,train_labels)

		# Save model
		pickle.dump(ab, open("../Models/ab_prob{}.model".format(i+1), 'wb'))

		if(i!=0):
			y_prediction_valid = y_prediction_valid + ab.predict_proba(validation_data)
		else:
			y_prediction_valid = ab.predict_proba(validation_data)

		y_prediction = ab.predict(validation_data)
		...
		#Train
		if(i!=0):
			y_prediction_train = y_prediction_train + ab.predict_proba(train_data)
		else:
			y_prediction_train = ab.predict_proba(train_data)

		# Test
		print("\n\nRunning test........................")

		if(i!=0):
			y_prediction_test = y_prediction_test + ab.predict_proba(test_data)
		else:
			y_prediction_test = ab.predict_proba(test_data)
...
csv_writer(t1, "../Results/ab_prob_valid.csv","wb")
csv_writer(t2, "../Results/ab_prob_train.csv","wb")
csv_writer(t3, "../Results/ab_prob_test.csv","wb")
