#NAIVE BAYES MULTINOMIAL : Using Probabilities
...
with open("../Results/nb_prob_multinomial_results.txt", "w") as text_file:
	...
	for i in range(5):
		...
		train_data = train_data + 80*np.ones((train_data.shape[0], train_data.shape[1]))
		test_data = test_data + 80*np.ones((test_data.shape[0], test_data.shape[1]))
		validation_data = validation_data + 80*np.ones((validation_data.shape[0], validation_data.shape[1]))
		...
		# Save model
		pickle.dump(nb, open("../Models/nb_multinomial_prob{}.model".format(i+1), 'wb'))
...
csv_writer(t1, "../Results/nb_prob_Multinomial_valid.csv","wb")
csv_writer(t2, "../Results/nb_prob_Multinomial_train.csv","wb")
csv_writer(t3, "../Results/nb_prob_Multinomial_test.csv","wb")
